...
use App\Http\Requests\TempOneRequest;
...
class PengandaanTenderUmumController extends Controller
{
    public function index($id)
    {
        $data['menus'] = MenuModel::find(base64_decode($id));
        $data['title']      = 'IFG | Pages';
        $data['title_page'] = 'Pengadaan Tender Umum | ' . $data['menus']->menu_name;
        $data['menu']       = MenuModel::all();
        $data['dataContent']       = ListItemModel::where('id_menu', base64_decode($id))->first();
        return view('pages.pengadaan_tender_umum', $data);
    }
    ...
    public function storePages(Request $req)
    {
        ...
        $log_app->request =  "Store Pengadaan Tender Umum";
        $log_app->response =  json_encode($response);
        $log_app->pages = 'Pengadaan Tender Umum';
