...
class IfgProgressController extends Controller
{
    public function index($id)
    {
        $data['menus'] = MenuModel::find(base64_decode($id));
        $data['title']      = 'IFG | Pages';
        $data['title_page'] = 'IFG Progress | ' . $data['menus']->menu_name;
    ...
    public function storePages(Request $req)
    {
        ...
        $log_app->request =  "Store IFG Progress";
        $log_app->response =  json_encode($response);
        $log_app->pages = 'IFG Progress';
